...
The initial text is starter copy, and the second project is explicitly a placeholder. Email and LinkedIn are blank until supplied. The live URL is https://tristan-de-halleux.netlify.app. Update `site.url` when connecting a custom domain; it controls canonical URLs, social metadata, RSS, and sitemap.
...
No environment secrets or SPA redirects required. Netlify's `COMMIT_REF` supplies the footer revision; local builds use Git. Native cross-document view transitions progressively enhance navigation; reduced motion is respected.
